...
Device-bound license: premium uses Telegram user ID as device ID.
"""
import hmac
import hashlib
import base64
...
SECRET_HEX = os.environ.get("SECRET_HEX", "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
SECRET = bytes.fromhex(SECRET_HEX)

# ========== STORAGE ==========
# Local JSON for dev/testing. For production cloud, replace with Supabase.
...
def parse_license(key, tg_id):
    key = key.strip()
    if key.startswith("ULTIMATE-FREE-"):
        try:
            b64 = key[len("ULTIMATE-FREE-"):]
    ...
    if key.startswith("ULTIMATE-PREMIUM-"):
        try:
            b64 = key[len("ULTIMATE-PREMIUM-"):]
            padded = b64 + '=' * (-len(b64) % 4)
            decoded = base64.urlsafe_b64decode(padded)
            if len(decoded) < 23:
